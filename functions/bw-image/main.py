...
def bw_image(event: dict[str, Any], *_) -> None:
  ...
  new_filename = "/".join(["processed"] + components[1:])
  print(f"Uploading to {new_filename}")
  storage_client.bucket(bucket).blob(new_filename).upload_from_filename(blurred_file)
